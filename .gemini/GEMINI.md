...
- まずプロジェクトのディレクトリ内に `docs/design` フォルダが存在するか確認してください。
- `docs/design` フォルダが存在する場合、その中にあるプロジェクトの仕様書を読み込み、仕様書に基づいて実装を行ってください。
  - 例えば シューティングゲーム用の仕様書だったら `shooting-game` というフォルダがあるはずです。
  - その場合、フォルダ内にある仕様書をすべて読み込み、仕様書に基づいて実装を行ってください。
    - requirements.md : 要件定義書  
    - design.md: 設計書
    - tasks.md: タスクリスト
  - 仕様書がない場合は、実装を開始する前に必ず `docs/design` フォルダを作成し、そのフォルダ内に仕様書を作成してください。
  - 仕様書を作成する順番は以下の通りです。
...
├── pages/                  # ページコンポーネント
│   ├── HomePage.tsx        # ホーム/スワップページ
│   ├── PoolsPage.tsx       # プール一覧ページ
│   └── PoolDetailPage.tsx  # プール詳細ページ
├── components/
│   ├── layout/
...
- [x] 1.2 React フロントエンドプロジェクトセットアップ
  - React プロジェクトの初期化
...
| カテゴリ         | 技術/ツール                                                              | 備考                               |
| ---------------- | ------------------------------------------------------------------------ | ---------------------------------- |
| **全体**         | pnpm, Node.js, ESlint, TypeScript                                         | パッケージ管理、実行環境、コード整形 |
| **スマートコントラクト** | Solidity, Hardhat, OpenZeppelin, viem, ethers                            | 開発、テスト、ライブラリ           |
| **フロントエンド**   | React               | フレームワーク、UI                 |
| **Web3 (フロント)**  | wagmi, viem, ethers                                          | ウォレット接続、コントラクト操作   |
...
3.  **フロントエンド開発**: `pkgs/frontend` 内で、Reactを使用してUIとロジックを実装します。
